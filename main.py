...
for instance in instances:
    ...
    boot_volume_info = compute_client.list_boot_volume_attachments(availability_domain,instance.compartment_id,instance_id=instance_id).data
    block_volume_info = compute_client.list_volume_attachments(availability_domain =availability_domain,compartment_id =compartment_id,instance_id=instance_id).data
    ...
    while True:
        ...
        if backup_status == "AVAILABLE":
            block_vol_backup=False
            for attached_vol_backup in attached_vol_backup_ocid:
                block_vol_backup_status = blockstorage_client.get_volume_backup(attached_vol_backup).data.lifecycle_state
                if block_vol_backup_status == "AVAILABLE":
                    block_vol_backup=True
                else:
                    block_vol_backup=False
                    print("Waiting for Attached volume backup to complete...")
            if not attached_vol_backup_ocid or block_vol_backup == True:
                break
